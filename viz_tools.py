...
def anim_1D(x,y, pas_de_temps, pas_d_images, save = False, myxlim = (0, 4) , myylim = (-4,4)):
    ...
    def animate(i):
        ...
        ax.set_title("$u(x)$ at = {} fs".format(np.round(i*pas_d_images*pas_de_temps*0.165, 4)), fontname = "serif", fontsize = 16)
        return line,
...
def anim_2D(X, Y, L, pas_de_temps, pas_d_images, save = False, myzlim = (-0.15, 0.15)):
    ...
    def update_surf(num):
        ...
        ax.set_xlabel("x [um]", fontname = "serif", fontsize = 14)
        ax.set_ylabel("y [um]", fontname = "serif", fontsize = 14)
        ax.set_zlabel("$u$ [u.a.]", fontname = "serif", fontsize = 16)
        ax.set_title("$u(x,y)$ at = {} fs".format(np.round(pas_d_images*num*pas_de_temps*0.165, 4)), fontname = "serif", fontsize = 16)
        ax.set_zlim(myzlim[0], myzlim[1])
        plt.tight_layout()
        return surf,
